...
fun main() = application {
    ...
    JBWindow(
        ...
        Column(Modifier.padding(4.dp)) {
            ...
            PackageSearchBox(
                pkgsViewModel.inputText, { pkgsViewModel.inputText = it },
...
sealed class Target(
            ...
            toolTipContent = {
                Column(Modifier.padding(vertical = 8.dp).fillMaxWidth(.3f)) {
                    Label(
                        ...
                        text = description.ifEmpty { "Contrary to popular belief, Lorem Ipsum is not simply random text. It has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old" })
    ...
    object MOBILE : Target("mobile", "", listOf(APPLE, ANDROID))
}
...
class PackageSearchViewModel() {
    ...
    init {
        searchResults.addAll(pkgSearchResultGenerator())
//        GlobalScope.launch {
...
fun SearchRow(
    ...
    Row(
        ...
        TextField(
            ...
            trailingIcon = {
                if (textSearchState.isNotEmpty())
                    Icon("icons/closeSmall.svg", modifier = Modifier.clickable { onTextValueChange("") })
            },
        )
...
fun pkgSearchResultGenerator(results: Int? = null): List<PKGSResult> {
    ...
    val targetsList = listOf(
        Target.ANDROID, Target.APPLE, Target.WATCHARM32, Target.IOSX64, Target.IOSARM64, Target.JS, Target.COMMON, Target.JVM
    )
    val targets = {
        buildList {
